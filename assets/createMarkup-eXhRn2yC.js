...
`,{handler:null,onShow(s){this.handler=Y.bind(s),document.addEventListener("keydown",this.handler)},onClose(){document.removeEventListener("keydown",this.handler),C.removeEventListener("click",E)}});f.show();const C=document.querySelector(".cross");C.addEventListener("click",E);function E(){f.close()}const y=document.querySelector(".modal"),a=document.querySelector("div.modal button.js-basket");U.some(s=>s.id===t)?a.textContent="Remove from basket":a.textContent="Add to basket";const c=document.querySelector("div.modal button.js-favorite");Q.some(s=>s.id===t)?c.textContent="Remove from favorite":c.textContent="Add to favorite",y.addEventListener("click",B);function B(s){if(s.preventDefault(),s.target.closest(".js-favorite")){if(S&&(g(s.target),c.textContent==="Add to favorite"?c.textContent="Remove from favorite":c.textContent="Add to favorite"),I){g(s.target);const u=JSON.parse(localStorage.getItem(v))??[];h(u,V),c.textContent==="Add to favorite"?c.textContent="Remove from favorite":c.textContent="Add to favorite"}k&&(JSON.parse(localStorage.getItem(v)),g(s.target),c.textContent==="Remove from favorite"?c.textContent="Add to favorite":c.textContent="Remove from favorite")}if(s.target.closest(".js-basket")){if(S&&(p(s.target),a.textContent==="Remove from basket"?a.textContent="Add to basket":a.textContent="Remove from basket"),k){p(s.target);const u=JSON.parse(localStorage.getItem(b))??[];h(u,q),a.textContent==="Remove from basket"?a.textContent="Add to basket":a.textContent="Remove from basket"}I&&(JSON.parse(localStorage.getItem(b)),p(s.target),a.textContent==="Remove from basket"?a.textContent="Add to basket":a.textContent="Remove from basket")}}}let X="Add to favorite",G="Remove from",W="Add to basket",Z="Remove from";const z="visible",tt="invisible";function h(i,e){let n;i.length?(n=i.map(({id:r,img:t,name:o,presentFuv:l,presentBas:f})=>`
    ...
    <button class="js-basket" type="button">${f?Z:W}</button>
                    ...
                    </li>`,e.innerHTML=n)}export{h as a,st as b,R as c,p as d,x as f,w as i,g as t};
//# sourceMappingURL=createMarkup-eXhRn2yC.js.map
